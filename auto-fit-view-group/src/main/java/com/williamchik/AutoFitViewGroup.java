...
import com.williamchik.autofitviewgroup.R;
...
public class AutoFitViewGroup extends ViewGroup {
  ...
  private int mRowMargin = getContext().getResources().getDimensionPixelSize(R.dimen.auto_fit_linear_layout_default_row_margin);

  /** 列间距，注意第一列的左方和最后一列的右方是没有间距的 */
  private int mColumnMargin =
      getContext().getResources().getDimensionPixelSize(R.dimen.auto_fit_linear_layout_default_column_margin);
  ...
  private void init(Context context, AttributeSet attrs) {
    ...
    mRowMargin = typeArray.getDimensionPixelSize(R.styleable.autoFitViewGroup_rowMargin, mRowMargin);
    mColumnMargin = typeArray.getDimensionPixelSize(R.styleable.autoFitViewGroup_columnMargin, mColumnMargin);
    typeArray.recycle();
  }
   ...
   * 数据变更后，通过 notifyDataSetChanged() 方法来更新数据
   */
